...
public:
	...
	Enemies(sf::Texture& t, sf::Vector2f f1, sf::Vector2f f2, std::unique_ptr<Move> m,  std::unique_ptr<CanMoveToPlace> c)
		:DynamicObj(t,f1,f2), m_move(std::move(m)), m_placeToMove(std::move(c)) {m_display.setOrigin(sf::Vector2f(15, 15));}
	~Enemies() {}
	virtual void move(Board& b)override { m_display.setPosition(m_move->move(b)); m_display.rotate();}
